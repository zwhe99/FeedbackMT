...
class RaftAlignerT2T(BaseAligner):
    """
    ...
    def _load_dataset(
        ...
        def preprocess_function(examples):
            with CaptureLogger(tok_logger) as cl:
                ...
                src_lang_indices = [tokenizer.lang_code_to_id[NLLB_CODE[ss]] for ss in examples["src_lang"]]
                tgt_lang_indices = [tokenizer.lang_code_to_id[NLLB_CODE[tt]] for tt in examples["tgt_lang"]]

                n_samples = len(inputs)
                tokenizer.tgt_lang = NLLB_CODE["en"] # @zwhe99: to avoid raising error
                model_inputs = tokenizer(inputs, text_target=targets, max_length=block_size, padding=padding, truncation=True)
    ...
    def _get_batch_dataset_local(
            ...
            for i, (query, src_lang, tgt_lang) in enumerate(zip(querys, src_langs, tgt_langs)):
                ...
                if rewards[idx_to_record] != -self.INF:
                    ...
                    record_src_langs.append(input_src_langs[idx_to_record])
                    record_tgt_langs.append(input_tgt_langs[idx_to_record])
            ...
            output_eval_dataset['instances'] = [{'input': ii, 'output': oo, 'src_lang': ss, 'tgt_lang': tt} for ii, oo, ss, tt in zip(all_inputs, all_outputs, all_src_langs, all_tgt_langs)]
            import json
            if local_rank == 0:
                with open(self.store_dir + "/train_set_" + str(iter_id) + ".json", 'w', encoding='utf8') as f:
