...
@dataclass
class MRTAlignerArguments(TrainingArguments):
    """
    Define a class MRTAlignerArguments to configure mrt aligner.
    """
    output_reward_path: Optional[str] = field(
        default="tmp/raft_aligner/",
        metadata={
    ...
    num_mrt_iteration: Optional[int] = field(
        default=20,
        metadata={
            "help": "number of iterations of the mrt aligner."
        },
    )
    mrt_batch_size: Optional[int] = field(
        default=1024,
        metadata={
            "help": (
                "only select {mrt_batch_size} samples each time for STF training."
            )
        },
    )
    mrt_alpha: Optional[float] = field(
        default=0.005,
        metadata={
            "help": (
                "MRT alpha to control the sharpness of the distribution of sampled subspace (default: 0.005)"
            )
        },
    )
    inference_batch_size_per_device: Optional[int] = field(
...
PIPELINE_ARGUMENT_MAPPING = {
    ...
    "raft_aligner_t2t": RaftAlignerArguments,
    "mrt_aligner_t2t": MRTAlignerArguments,
}
